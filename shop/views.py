...
def index(request):
    ...
    Utsav = {'allProds': allProds}
    return render(request, 'shop/index.html', Utsav)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    Utsav = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        Utsav = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', Utsav)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            Utsav_dict['CHECKSUMHASH'] = Checksum.generate_checksum(Utsav_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'Utsav_dict': Utsav_dict})
